...
With Empirical Evidence from RDNA3 ISA Research, ML Training Logs, Accessibility Patterns, and Healthcare Systems Analysis  
...
Accessibility built in from the start is a quality multiplier, not a compliance checkbox. It reduces rework costs by orders of magnitude, improves AI model extraction accuracy, expands audience reach to more than a billion disabled people worldwide, and consistently outperforms inaccessible alternatives across every measurable dimension. These are not separate benefits; they are the same benefit expressed at different layers of the same system.

This thesis advances a unified argument: accessibility reduces ambiguity, and ambiguity reduction is the mechanism by which both human readers and machine systems achieve accurate, reliable output. Evidence for this claim is drawn from six domains—healthcare, software engineering, UX design, AI/ML, documentation pipelines, and government services—and is grounded in empirical data from twelve documented ML training runs on consumer AMD RDNA3 hardware. Those training runs revealed, at the instruction‑set level, the same principle that governs accessible content design: when structure is absent, systems do not fail loudly. They continue, silently wrong.
...
- [§12 Conclusion: The Constraint That Makes Everything Else Work](https://github.com/thejeangenie18/rocm-7700xt-qlora/blob/main/research/new-research.md#12-conclusion:-the-constraint-that-makes-everything-else-work)
...
Accessibility is not charity. It is the highest‑ROI design decision available at project inception, and the only one that simultaneously reduces legal exposure, expands the addressable market, improves AI and machine‑parsing performance, and cuts downstream maintenance costs. Every other architectural choice affects a subset of these outcomes. Accessibility affects all of them at once. 

The central argument of this thesis is precise: accessibility reduces ambiguity, and ambiguity reduction is the mechanism by which both human readers and machine systems achieve accurate, reliable output. This is not metaphorical. It is a structural claim, and it holds at every layer of the stack—from the semantic structure of a web interface, to the instruction‑stream typing of a GPU compute unit, to the memory‑fence sequencing of a machine‑learning kernel. Systems that receive well‑typed, unambiguous input produce correct output. Systems that receive ambiguous input continue operating and produce output that appears correct but is not.
...
The scale of the market begins the financial case. More than 1.3 billion people worldwide live with a disability, representing trillions in collective spending power. Industry research shows that organizations implementing accessible technologies report improved customer experience, reduced risk, and measurable operational gains. These are not edge‑case outcomes. They are the documented returns on a decision that most organizations still treat as optional.

The author of this thesis is Deaf, autistic, ADHD, and hypermobile. The communication patterns, access needs, and executive‑function challenges that characterize this profile do not match the “default user” most AI systems are designed around. That gap—between the technical sophistication required to diagnose what is going wrong and the lived vulnerability of the people who bear the consequences—is precisely where both engineering and policy must intervene.

The RDNA3 ISA research documented here emerged from a direct attempt to build tools that work for disabled and neurodivergent people on consumer hardware. What began as a question about why a kernel was hanging became a systematic extraction of ISA‑level rules that few frameworks, tutorials, or production AI pipelines on consumer AMD hardware follow correctly. The parallel to accessibility is not incidental. It is structural. Both domains share the same failure mode: ambiguity inserted at the authoring stage propagates downstream and compounds. Structure is not cosmetic. It is the signal the system runs on.

*Sources: Forrester Research, 2016; nascio.org, 2024; accessibility.works, 2025; AMD RDNA3 ISA Reference, 2023*

---

## §2 The Retrofit Tax: Why Deferred Accessibility Always Costs More

Most organizations treat accessibility as a post‑launch audit item, something to address once the product is live and complaints have begun arriving. That sequencing decision is not a minor process inefficiency. It is the root cause of every cost explosion, every lawsuit, and every six‑month remediation sprint that consumes the roadmap for the following year.

The problem is not a lack of concern for accessibility. The problem is that accessibility is deferred to a phase where fixing it is structurally expensive. Understanding why requires examining the documented cost structure of deferred work — and the legal exposure that accelerates it.

### 2.1 The 1-10-100 Rule

Industry analyses of post‑launch remediation are unequivocal: fixing accessibility issues after launch costs an order of magnitude more than addressing them during initial development. The further downstream a defect travels, the more systems it touches, the more documentation it contaminates, and the more expensive it becomes to unwind. This is not unique to accessibility — it is the documented behavior of any architectural defect that propagates before being caught.

The structural cost model is known as the 1‑10‑100 rule:  
- a defect costs 1x to fix at the design phase,
- roughly 10x during implementation or testing,
- and roughly 100x under legal or post‑release pressure.

Accessibility defects follow this rule with particular severity because they tend to be architectural,embedded in templates, component libraries, and content pipelines, rather than isolated to a single feature.

A missing structural requirement in a shared component is not one defect. It is every instance of that component, across every page, across every user session, compounding silently until someone files a complaint or a lawsuit.

> **HARD CONSTRAINT:  **  
> Every sprint that ships inaccessible code is a future sprint of debt.
> The 1‑10‑100 rule is not a theory. It is the documented cost structure of deferred accessibility work across industries.
> Accessibility is not a feature toggle. It is a load‑bearing structural property. Defer it and the debt compounds; it does not wait.

### 2.2 Legal Exposure Is Accelerating

Digital accessibility lawsuits in the U.S. surged more than 300% from 2018 to 2023. A single lawsuit can cost hundreds of thousands of dollars in legal fees alone — before remediation costs, before reputational damage, and before the forced architectural overhaul that typically follows a settlement. This is not a niche compliance risk manageable by a legal checkbox. It is the mainstream market reality, and it is accelerating as AI search tools increasingly penalize unstructured content at the discovery layer.

The solution is not more auditing after the fact. It is structural: accessibility must be embedded in the information architecture from the first keystroke. The same principle holds at the hardware level. RDNA3’s ISA rules are not suggestions appended to a working system; they are the conditions under which the system produces correct output at all. Violate them and the system does not stop. It continues, silently wrong.

The retrofit tax applies equally to GPU kernels and to web applications: the cost of fixing a structural violation after the system is in production is always a multiple of the cost of building it correctly from the start.

--- 

**Cost Differential Across Common Accessibility Features ** 

| **Feature** | **Build‑In Cost** | **Retrofit Cost** | **Multiplier** |
| --- | --- | --- | --- |
| Color contrast | Design token choice (~0 dev time) | Component library audit + redesign | 10–30× |
| Text alternatives | Written at upload (~seconds per asset) | Audit + retroactive writing for all archived assets | 20× |
| Structural labeling | Added during component build | Post‑hoc instrumentation of entire UI surface | 15–25× |
| Accessible document template | Configured once; all exports inherit | Document‑by‑document recreation and tagging | 5–15× |
| Form error handling | Built into initial UX spec | UX research + redesign + full retest cycle | 8–20× |

**Table 1.** Build‑in vs. retrofit cost comparison for common accessibility features. Multipliers reflect documented remediation cost ratios.

*Sources: WebAIM via eajournals.org; inspekter.com, 2025*

---

## §3 Accessibility as Structure: Semantic Markup as Shared Infrastructure

Accessible content is structurally sound content. The core practices of accessibility—semantic HTML, clear heading hierarchy, descriptive text alternatives, meaningful link labels, and consistent structural cues—are not bolt‑on features added after the “real” architecture is complete. They define the information architecture that both humans and machines depend on. When accessibility is omitted, it is not only users with disabilities who encounter barriers. Every downstream system that attempts to parse, index, summarize, or route the content inherits the same structural gaps. 

This is the insight that connects accessibility to AI behavior: the structural properties that make content accessible to assistive technology users are the same properties that make content parseable by search crawlers, large language models, and voice interfaces. These are not separate optimizations. They are the same optimization, applied at the same layer of the stack, producing benefits that propagate upward through every system that consumes the content. Section 4 quantifies those benefits in measurable AI‑performance terms.

### 3.1 Semantic Structure as Shared Infrastructure

A 2025 SEMrush analysis of 10,000 websites quantified the traffic consequences of this structure gap directly: accessible, standards‑compliant sites gained 23% more organic search traffic and ranked for 27% more keywords than non‑compliant peers. Non‑accessible sites lost up to 20–30% of their traffic to emerging AI‑driven search tools that favor well‑structured content—not because the content was worse, but because the structure made it unreadable to the systems that now mediate discovery. The content existed. The signal did not.
...
*Sources: accessibility.works, 2025; 365 Researcher ROI Digest, 2025*
...
The most instructive case is VOPD (dual‑issue vector operation). VOPD is a wave32‑only instruction that doubles throughput for specific vector pairings. In wave64 mode, VOPD instructions are silently skipped—no error, no diagnostic, no half‑speed warning. The instruction simply does not execute. The kernel continues. The output looks normal. The throughput loss is invisible until someone with the right knowledge examines the profiler output.
...
A 2025 XBRL study on large‑language‑model retrieval from corporate financial reports measured this directly. When financial data was provided in structured, accessible XBRL format rather than unstructured text or HTML, the error rate for key metrics dropped from 15–29% (plain text/HTML) to approximately 7–9% (XBRL), a 40–75% reduction in extraction errors. The most dramatic improvement came from scaling errors—misreading units such as “millions” vs. “billions”: these fell from roughly 8% in unstructured text to just 0.11% with XBRL’s context tags. The content was identical. The structure was not.

A 2025 systematic review of knowledge‑graph‑augmented LLMs confirmed the same directional finding at a broader level: integrating structured knowledge graphs significantly improves LLM accuracy, reduces hallucinations, and increases reasoning explainability. The mechanism is identical to what the XBRL study found: structure reduces the search space the model must navigate, increasing confidence and decreasing error. Ambiguity is not a neutral property of input. It is a tax on every inference operation the model performs.

Search engines and AI models parse content through mechanisms directly analogous to assistive technologies: all require the same structural signals to route, weight, and interpret content accurately. Building for accessibility and building for AI are not different tasks. They are the same task, applied to the same structural layer, producing benefits that propagate to every consumer of the content.

*Sources: Sources: xbrl.us (2025); ACL Anthology (2025); accessibility.works (2025).*

---

| **Accessibility Feature** | **AI Benefit When Present** | **Degradation When Absent** |
| --- | --- | --- |
| Semantic heading hierarchy | Correct document outline extraction; accurate section weighting | Flat text blob; misranked content |
| Tables with scoped headers | Accurate row/column parsing and data attribution | Misattributed values; row/column confusion |
| Text alternatives for images/charts | Visual information becomes parseable text | Image content opaque to the model |
| Descriptive link text | Link intent and destination are machine‑interpretable | “Click here” carries zero semantic value |
| Logical reading order | Content parsed in author‑intended sequence | Layout vs. reading order mismatch; mangled output |
| Document regions/landmarks | Sections identifiable for differential weighting | Undifferentiated content; navigation treated as body text |
| Structured data formats (e.g., XBRL) | 40–75% fewer extraction errors | Scaling errors, unit misreads, silent data loss |

**Table 2.** Accessibility features and their direct AI‑performance impact.

> **KEY FINDING:**
> Accessible design is increasingly synonymous with AI/SEO optimization. Both search engines and AI agents parse content much like assistive technologies. Building for accessibility is building for the machines that are increasingly the first readers of your content. Accessible structure is not a human accommodation. It is the input format the entire modern information stack is optimized for.
...
- **VScnt** (vector memory stores)
- **LGKMcnt** (LDS, scalar memory, and flat operations)
- **EXPcnt** (exports)

These counters are independent. **S_WAITCNT does not drain VScnt**. A kernel that writes results and terminates without explicitly draining VScnt may produce incomplete output silently.

The write appears to succeed. The system advances. The receiving layer operates on stale state. This is the hardware equivalent of an accessibility gap in a documentation pipeline. In both cases, the failure is invisible at every abstraction layer above the point of origin. It only becomes visible when someone with the right knowledge—a hardware engineer reading ISA output, or a disabled user reading a document that was supposed to accommodate them—looks closely enough to notice.
...
Early accessibility implementation is not a cost. It is cost avoidance at scale. Every dollar invested at the design phase eliminates $10–$100 in downstream remediation, litigation, extended QA cycles, and support volume. This pattern does not vary by team size, budget, or industry. The only variable is when you pay — and the later you pay, the more you pay. 
...
Teams that embedded accessibility into development cycles from the beginning saw a ~27% improvement in development efficiency and a ~35% reduction in long‑term maintenance costs relative to teams that retrofitted accessibility after release. These gains arise from the same discipline that produces accessible output: standardized structure, predictable patterns, and reduced ambiguity. 

Accessibility is not an additional constraint on top of good engineering. It is the constraint that enforces good engineering. Quality metrics track in the same direction: organizations practicing inclusive engineering reported ~15% fewer JavaScript errors, ~22% fewer runtime exceptions, and a ~25% reduction in page load times.

Accessible code is cleaner code. The constraints of accessibility — clear structure, predictable behavior, deterministic state — impose the same discipline that produces robust systems generally.

*Source: eajournals.org (2025).*

### 5.2 User Experience and Revenue

User experience outcomes are equally concrete. Companies implementing comprehensive accessibility guidelines saw a **63% increase in overall user satisfaction** and **37% higher task‑completion rates** across digital platforms. Task completion is not a soft metric; it is a direct revenue driver. Every user who cannot complete a task is a transaction that did not occur.

The revenue case is documented at the organizational level. One financial services firm attributed **£13 million in added annual revenue** to conversion improvements following accessibility implementation across their platform. In e‑commerce, accessible interfaces reduced abandonment rates by **~13%** and increased successful transactions by **~17%**.

*Sources: eajournals.org (2025); Accenture (2018).*

### 5.3 Market Scale and Legal Risk

More than **1.3 billion people** worldwide live with a disability, collectively controlling **trillions in spending power**. Organizations that build accessibly from the start gain access to this customer base without additional spend. Organizations that do not build accessibly exclude it entirely, often without realizing it until the exclusion appears as a lawsuit, a churn spike, or a support anomaly that cannot be explained by any other variable.

Legal exposure is accelerating. Digital accessibility lawsuits in the U.S. increased by over 300% from 2018 to 2023. A single lawsuit can cost up to $350,000 in legal fees alone, before remediation costs, reputational damage, or the architectural overhaul that typically follows a settlement.

This is the **1‑10‑100 rule** expressed in litigation: the cost of fixing a structural violation after it has been litigated is always a multiple of the cost of building it correctly from the start.

*Sources: nascio.org (2024); eajournals.org (2025).*

---

## §6 Real-World Evidence: Healthcare, Engineering, and Government

The economic case for accessibility‑first design is not theoretical. It is documented in operational budgets, patient outcomes, defect rates, and revenue figures across multiple sectors. Two sectors—healthcare and software engineering—illustrate the compounding return on early accessibility investment with particular clarity, because in both cases the cost of inaccessibility is measured in concrete, auditable outcomes rather than estimates.

### 6.1 Healthcare: Accessibility as Operational Infrastructure

The U.S. Social Security Administration’s transition to accessible online services produced a measurable operational result: millions in cost savings as in‑person visits and call‑center volume declined, particularly among seniors and users with limited digital proficiency. Accessibility did not add cost to service delivery. It reduced it by enabling users to complete tasks independently that previously required staff assistance. 

Accessible patient portals correlate with concrete clinical and economic outcomes. Systems that are navigable and structurally clear show:
- fewer office visits,
- better medication adherence,
- higher patient satisfaction,
- and lower overall care‑cycle costs.

The mechanism is direct: when patients can access records, manage prescriptions, and communicate with care teams without staff intervention, the associated labor and facility costs are recaptured.
...
The Minnesota Department of Health and Human Services provides a case study in proactive accessibility embedding. By incorporating accessibility requirements from the project’s outset—rather than as a post‑launch audit—the agency avoided expensive retrofits and delivered a smoother experience for all users. Project leaders reported clearer documentation, better vendor performance, and more predictable delivery timelines. The accessibility requirement reduced ambiguity and improved procurement outcomes across the board.

In digital banking, one accessible self‑service implementation reduced call‑center workload by **up to 40%**, lowering operational costs accordingly. The reduction was not due to fewer customers. It was due to customers being able to complete tasks independently. Accessibility, in this context, is not a cost center. It is a capacity multiplier.

*Sources: nascio.org (2024); adoc‑studio.app (2025).*

### 6.2 Software Engineering: Accessibility as Code Quality

Teams practicing proactive accessibility saw a **~27% improvement in development‑cycle efficiency** and a **~35% reduction in long‑term maintenance costs**. These gains are not attributable to accessibility work alone—they arise from the discipline that accessible development requires. Semantic, well‑structured code is inherently more maintainable, more testable, and more resistant to regression than code written without structural constraints.

Accessibility is not an additional constraint on top of good engineering. It is the constraint that enforces good engineering.

Inclusive engineering correlates with fewer overall defects across the entire codebase:  
- **~15% fewer JavaScript errors**
- **~22% fewer runtime exceptions**
- **~25% reduction in page‑load times**

The discipline of writing accessible code—clear structure, predictable behavior, deterministic state—produces cleaner, more robust architectures. These are not separate wins from the accessibility work. They are the same win, because the structural properties that make code accessible are the same structural properties that make code maintainable.

The innovation spillover from accessibility work has historically produced many of the most widely adopted features in consumer technology. Voice interfaces, closed captions, responsive design, predictive text, and autocomplete all originated as accessibility solutions before becoming mainstream. The pattern is consistent: solving for the constrained use case produces solutions that generalize. The constraint is not a limitation. It is the design pressure that produces better outcomes for all users.
...
The AMD RDNA3 Instruction Set Architecture provides the most technically precise evidence available for the central claim of this thesis: ambiguity, inserted at the structural level, propagates downstream and compounds. The ISA documents a set of rules that, when followed, produce correct output at rated throughput. When violated, the system does not stop. It continues, silently wrong. The confidence of the output tells you nothing about its correctness.

This is not a metaphor for accessibility failures. It is the same failure mode, expressed at the hardware level. The research documented in this section emerged from a direct attempt to run QLoRA training on an RDNA3 GPU (Radeon RX 7700 XT) using ROCm 7.2.1. What began as a question about why a kernel was hanging became a systematic extraction of ISA‑level rules that virtually no framework, tutorial, or production AI deployment pipeline on consumer AMD hardware follows correctly. The findings are documented across twelve training runs spanning May–June 2026, and they are reproducible.
...
A document pipeline that does not enforce section hierarchy produces output that looks structured but is not. Downstream parsers, human or machine, receive corrupted signal with no indication that anything is wrong.
...
**Accessibility parallel:**  
A structural label present in markup but not propagated through the processing pipeline is the same architectural failure — the label exists, the system advances, and the receiving layer operates on stale state.
...
**Accessibility parallel:**  
A partial compliance audit that checks individual components without verifying end‑to‑end propagation is a partial wait, the system advances on the assumption that the operation completed when it did not.
...
**Accessibility parallel:**  
A grievance or accommodation request that is logged but not propagated to operational teams is the same architectural failure, the write appears to succeed, the system advances, and the receiving layer operates on stale state.  
...
**Accessibility parallel:**   
A user‑need flag recorded in one system but not propagated downstream results in silent data loss. Each stage completes “successfully.” The information never arrives.

### VOPD Silently Skipped in Wave64

VOPD is wave32‑only. In wave64 mode, VOPD instructions are silently skipped — no error, no diagnostic, no warning.

**Accessibility parallel:**  
Inaccessible content is silently excluded from AI discovery. The content exists. The system simply does not read it.

| **ISA Hazard** | **Hardware Effect** | **Accessibility Parallel** |
| --- | --- | --- |
| Missing WMMA V_NOP bubble | Silent wrong matmul results | Missing section hierarchy: downstream parsers receive a corrupted document outline |
| EXEC mask not restored before WMMA | Incorrect attention scores | Structural label present in markup but not propagated: false signal of correctness |
| FLAT partial wait (not waitcnt 0) | Race conditions; stale reads | Partial accessibility audit: downstream failures remain undetected |
| S_BARRIER without lgkmcnt drain | Data race in tile kernels | Grievance or update filed but not implemented: operational state becomes stale |
| VScnt not drained before kernel end | Stores not visible across kernels | Accommodation or user‑need flag logged but not propagated: silent data loss |
| VOPD silently skipped in wave64 | Invisible throughput loss | Inaccessible content silently excluded from AI discovery and indexing | 
...
After twelve documented training runs, a fully RDNA3‑safe QLoRA pipeline was established. The configuration that produces stable, correct results requires:  
- BF16 compute with FP32 accumulators
- No Triton kernels
- No FlashAttention
- ROCm‑native matmuls only (rocBLAS/hipBLAS)
- Standard PEFT QLoRA adapters with no fused kernels
...
The final training run, a full 3‑epoch QLoRA cycle on a 3B‑parameter model, 11,764 training examples, 2,208 steps, achieved a starting loss of 2.711 and a final loss of 0.18–0.21 with no GPU faults and no silent failures. Reaching that point required reading the hardware ISA and implementing fixes at a level of abstraction that no high‑level framework surfaces.

The accessibility parallel is exact: reaching a fully accessible system requires reading the structural requirements and implementing them at the level of the information architecture, not the surface layer. The fix is always structural. The surface layer is always downstream of the problem.

ROCm 7.2.4 subsequently introduced kernel‑correctness updates that partially address several of the documented RDNA3 ISA‑level hazards, including improved WMMA/MFMA dependency handling, corrected EXEC mask restoration, more complete waitcnt insertion for FLAT/SMEM, and improved LDS access patterns. These changes confirm that the hazards described were architectural rather than user error — exactly as the ISA analysis predicted.

When the structural rules are followed, the system produces correct output. When they are not, it does not.

*Sources: RDNA3 ISA Research Compilation, June 2026; README.md, June 2026; Appendix A*

---

## §8 AI Brittleness in Clinical Contexts: When Silent Failures Meet Healthcare

Healthcare is now one of the fastest‑moving domains for AI deployment. It is also, by every measure, the domain where silent failure is most dangerous. The brittleness documented at the hardware level — systems that continue operating and produce output that looks correct but is not — has direct structural equivalents in how healthcare AI systems are built and operated. These failure modes are not hypothetical. They are documented patterns, and they are the exact patterns that AI is currently being deployed to automate.
...
The following twelve patterns are drawn from documented, systemic issues across healthcare administration, clinical care coordination, and digital access. Each maps precisely to a class of AI failure documented at the hardware level. The structural logic is identical: a system that was not designed to handle the full range of inputs it receives will fail silently at the edges of its training distribution.

#### 1. Diagnostic Silos in Complex Multi-System Conditions

Systemic issue: Healthcare systems organized by specialty create structural blind spots for patients whose conditions span multiple body systems.  
AI parallel: A model trained on single‑specialty datasets inherits the same silo. It routes correctly within its distribution and fails silently at the edges — like a kernel that exceeds its tile boundary without a bounds check.

#### 2. Accommodation Documentation That Does Not Travel

Systemic issue: Accessibility accommodations recorded at scheduling rarely reach the point of care.  
AI parallel: A scheduling model that logs a flag but does not propagate it downstream has a silent data‑loss event between pipeline stages — identical to a store instruction no later stage ever reads.

#### 3. Grievance Resolutions That Are Not Implemented

Systemic issue: Formal resolutions issued by patient‑advocacy departments rarely propagate to operational teams.  
AI parallel: This is the memory‑fence problem where a write that appears to succeed but is not visible to the next layer.

#### 4. Phone-Based Communication as Structural Exclusion

Systemic issue: Healthcare defaults to phone‑based reminders and coordination — a modality structurally inaccessible to Deaf patients.  
AI parallel: A model trained on hearing‑centric workflows generates phone‑call prompts as its default “accommodation,” producing a fluent hallucination of compliance.

#### 5. Digital Health Platforms That Exclude Disabled Users

Systemic issue: Patient portals often require phone numbers, omit email‑only pathways, or break screen‑reader navigation.  
AI parallel: A model optimized for portal completion defines “accessible” as “works for the users who complete it.” Disabled users who cannot complete the portal are absent from the signal.

#### 6-12. Additional Structural Failure Patterns

The remaining patterns follow the same structural logic:  
- Referral loops with no ownership → routing systems with no terminal state.
- Diagnostic overshadowing → biased training data amplified, not corrected.
- Reactive financial assistance → capabilities not invoked by default are invisible to the model.
- Childhood findings not connected to adult care → short context windows producing confident omissions.
- Normalized lab trends → accuracy at a single moment mistaken for accuracy across time.
- Unauthorized record edits → prompt‑injection vulnerabilities in clinical workflows.
- Compounding failures → systems that evaluate errors individually cannot see the pattern.

> **PLAIN-LANGUAGE TAKEAWAY:**
>     Healthcare AI does not need to fail catastrophically to cause harm. It only needs to be wrong at the wrong moment, in the wrong direction, with enough fluency that no one questions it. These are not hypothetical risks. They are documented structural patterns — and they are the exact systems AI is currently being deployed to automate.

### 8.2 Industry Validation: GitHub's Accessibility Agent Findings

GitHub, one of the most sophisticated AI‑first engineering organizations in the world, published findings on deploying AI agents to identify and fix accessibility issues in code. Their conclusion: **AI agents cannot autonomously fix accessibility. Human oversight is structurally required.**

The core finding is that accessibility requirements are deeply contextual, involve tradeoffs that cannot be resolved by pattern‑matching, and require judgment informed by lived experience of disability — experience that is not captured in the text and code models are trained on.

An AI agent can identify that a button is missing a label.  
It cannot determine whether the label it generates is meaningful to a screen‑reader user navigating a complex form.  
It cannot know whether the error message it wrote is interpretable to a Deaf user reading in ASL‑influenced English.  
It cannot know whether the workflow it proposes assumes a phone call a Deaf person cannot make.

Every one of these failures is a version of the silent wrong result documented at the hardware level: correct‑looking output, wrong meaning, no flag.

If the engineers building the tools acknowledge that AI cannot autonomously handle accessibility, the rest of the industry — especially healthcare administrators deploying these systems — must hear that clearly.

*Sources: GitHub, “Building GitHub’s Next Chapter in Accessibility”; LinkedIn Article (June 2026).*
...
This is not a coincidence. It is the predictable outcome of designing systems without the input of the people who will reveal their failure modes. The framework proposed here is organized around the same principle that makes the RDNA3 ISA work: explicit rules, enforced at the right level of abstraction, that prevent silent failures from propagating downstream. Policy oversight is the instruction set for AI deployment. The rules must be written, published, and required before the silent errors accumulate into something irreversible.

### 9.1 Five Structural Requirements

#### 1. Mandatory Pre-Deployment Accessibility Testing with Disabled Users

Not with a checklist. Not with a simulation. With actual Deaf users, actual autistic users, actual users with cognitive and physical disabilities, in their real workflows, using their real communication patterns. If a system cannot be tested this way before deployment, it should not be deployed.  
...
Silent wrong answers must be treated as safety‑critical defects — the same way a missing dependency bubble in a WMMA sequence is treated as a correctness failure, not a performance suggestion. 

#### 4. Communication-Accommodation Compliance in AI Interfaces

Any AI system that interacts with patients, benefits recipients, or members of the public must offer written‑only communication pathways. Phone‑only escalation paths in AI‑mediated workflows violate accessibility law for Deaf users. This is not a future regulation. It is current law, and it is currently being violated by deployed systems.

The fix is structural: build the written pathway in from the start, or pay the retrofit multiplier when the violation is litigated.
...
Transparency is the only scalable auditing mechanism.
...
> **PLAIN‑LANGUAGE TAKEAWAY:**  
> AI is not too complicated for meaningful oversight. Every failure mode documented at the hardware level has a corresponding rule in the ISA that, when followed, prevents it. Policy oversight is the instruction set for AI deployment. We need to write it, publish it, and require people to follow it before the silent errors accumulate into something irreversible.
---

## §10 Accessibility‑Structured Documents Produce Exceptionally Clean Training Data
First Case Study   

During the development of a personal engineering assistant model, I generated a structured JSONL dataset from a multi‑page accessibility training document. The source was pre‑processed into a clean PDF (backgrounds removed, no headers/footers) and ingested using Microsoft’s 365 Office Agent Frontier dataset‑extraction workflow.

**Key Result:**  
Frontier produced a fully valid dataset with zero JSON errors, no invented attributes, and no fabricated patterns.
All extracted code examples were reproduced exactly as they appeared in the source material.

**Extraction Outcomes**  
Frontier successfully identified and converted a wide range of pattern‑based examples, including:
- structural patterns
- interaction behaviors
- labeling and description techniques
- state‑based logic
- component constraints
- assistive‑technology considerations
- distinctions between structural and rendered hierarchies

These were transformed into clean, grounded training samples without hallucination or drift.

**Why This Matters**  
This experiment demonstrates that accessibility‑aligned engineering content produces exceptionally clean machine‑learning training material.
Its inherent structure:  
- reduces ambiguity
- minimizes hallucination risk
- improves extraction fidelity
- increases reasoning quality
- yields deterministic training examples
...
This dataset is now part of the training corpus for the personal engineering assistant model.Expected benefits include:  
- improved reasoning about structured patterns
- more accurate synthesis of accessible components
- better repair of flawed examples
- reduced hallucination of attributes or behaviors
- stronger cross‑technology reasoning
...
To maintain transparency and allow independent verification of the extraction workflow, the sanitized prompt used to generate the dataset is included here:  
[Frontier Extraction Prompt](https://github.com/thejeangenie18/rocm-7700xt-qlora/blob/main/research/prompts.md)
...
**Image Description:**  
The image shows a dark‑themed computer screen containing a block of instructional text for an accessibility‑focused data‑extraction task. The text explains that the user must convert a 52‑page training PDF into JSONL training data for a coding‑assistant model, following a strict schema with “instruction,” “input,” and “output” fields. The instructions stress that no CUDA, Triton, FlashAttention, GPU logic, device maps, training code, or optimization code may be introduced, and that no examples, attributes, or code may be invented. The text states that all output must be deterministic and must come directly from the PDF. It then describes the first two categories of training samples to generate: repair samples, which require fixing accessibility issues in code snippets taken from the PDF and returning corrected code with a short explanation, and critique samples, which require listing accessibility issues in a snippet and explaining why each issue matters. The visible portion ends mid‑sentence as it continues describing the critique sample format.

2. Automated PDF Parsing and JSONL Conversion Sequence

![A dark‑themed interface labeled "Office Agent" shows an automated workflow that parses an accessibility PDF, extracts patterns, converts them into JSONL training data, and executes several Python commands. The interface displays step‑by‑step logs, file reads, and a final message indicating the task is complete.](../evidence/2026-06-17-prompt2.png)  
**Alt Text:**  
A dark‑themed interface labeled “Office Agent” shows an automated workflow that parses an accessibility PDF, extracts patterns, converts them into JSONL training data, and executes several Python commands. The interface displays step‑by‑step logs, file reads, and a final message indicating the task is complete.  
**Image Description:**  
The image shows a dark‑themed screen titled “Office Agent,” presenting a sequence of automated steps for processing accessibility training material. The interface displays a series of status messages describing how the system parses an accessibility PDF, extracts patterns, and converts them into JSONL training data. Several lines show Python commands being executed, including reading intermediate files such as `pdf_content.txt` and running small inline scripts to process extracted text. The interface shows each stage completing successfully as the agent moves through parsing, chunking, extraction, and conversion. At the bottom of the screen, three rectangular buttons appear: one labeled “Convert into workflow,” another labeled “Expand coverage to more patterns,” and a third labeled “Analyze training data quality and balance.” The sequence ends with a confirmation that the task has completed.

3. Automated JSONL Generation and Validation Sequence

![A dark‑themed interface shows an automated workflow generating and validating a JSONL dataset from extracted accessibility patterns. The screen displays file creation steps, Python command execution, validation messages confirming all records are correct, and a final summary of dataset counts.](../evidence/2026-06-17-prompt3.png)  
**Alt Text:**  
A dark‑themed interface shows an automated workflow generating and validating a JSONL dataset from extracted accessibility patterns. The screen displays file creation steps, Python command execution, validation messages confirming all records are correct, and a final summary of dataset counts.  
**Image Description:**  
The image shows a dark‑themed screen displaying an automated sequence of steps for generating and validating JSONL training data. The interface reports that it is creating two files named `todo.md` and `gen_jsonl.py`, then running a Python script to produce the dataset. After generation, the system executes another Python command to verify that all JSONL records are valid, followed by a spot‑check of sample entries from each training‑sample type. The log confirms that all seventy‑eight records pass validation. The interface then rewrites the todo.md file and states that the dataset `training.jsonl` is ready. At the bottom, a small summary table appears, showing the total number of records and the counts for repair, critique, synthesis, and explanation samples. The screen ends with a clear indication that the dataset creation process has completed successfully.

## §11 Accessibility Structures Improve LLM Determinism

### Overview

During the Apex 4X corpus experiment, a surprising and repeatable pattern emerged:  
**LLMs behave more deterministically, more conservatively, and with dramatically fewer hallucinations when operating over accessibility‑structured source material.** 

This section documents the second major empirical confirmation of the thesis:

> **Accessibility benefits everyone, including large language models.**

The results were produced using a two‑agent pipeline (one agent for extraction, one for dataset synthesis) operating over a highly structured corpus of 681 modules.

All copyrighted source materials were processed internally by LLM agents and are not reproduced here.
All prompts, methodology, analysis, and results are original research outputs.

### Methodology Summary  

Methodology Summary
1. An extraction agent was instructed to walk a large, accessibility‑structured codebase and emit one Markdown file per module.
2. All 681 Markdown files were merged into a single corpus file.
3. A dataset‑generation agent ingested the merged corpus using a tri‑mode prompt (synthesis, repair, critique).
4. The resulting JSONL dataset was validated for:
...
No copyrighted text was included in the dataset or this document.
...
- Total records: 125
- File size: ~167 KB
- Validation: 100% valid JSON; no empty fields; no markdown leakage
- Average output length: 932 characters
- Grounding: 100% grounded in corpus; zero invented APIs or behaviors
...
- System lifecycle components
- Widget and interface modules
- Keyboard interaction logic
- Template system structure
- Tutorial examples
- Accessibility‑related utilities

Even extremely large modules were handled conservatively, with no invented content.

**Ambiguity Handling**  

A key finding:  
**Whenever the corpus contained ambiguity, truncation, or missing context, the LLM agents defaulted to conservative behavior rather than hallucination.**

Examples include:  
- Large files → extracted via targeted ranges
- Third‑party wrappers → noted, not expanded
- Sparse modules → documented without invention
- Binary assets → excluded
- Missing documentation → acknowledged, not fabricated

This behavior is atypical for LLMs and strongly suggests that accessibility‑structured corpora reduce hallucination pressure.

**Interpretation**  

The results support the central thesis:  
> **Accessibility structures improve LLM determinism.**

Accessibility‑aligned systems share properties that LLMs handle exceptionally well:  
- semantic clarity
...
These properties reduce ambiguity and push the model into a more grounded, rule‑following mode.
...
This is the second independent empirical confirmation of the thesis, following earlier experiments with accessibility materials.

---

## §12 Conclusion: The Constraint That Makes Everything Else Work
...
And now, with the structured‑document → Frontier → JSONL case study, the same pattern is visible in AI systems. A multi‑page accessibility‑aligned training document produced a perfectly clean dataset: 78 records, zero validation errors, no invented attributes, and no fabricated examples. The reason is the same mechanism observed everywhere else: accessibility enforces structure, and structure is what makes machine reasoning reliable. When content is deterministic, hierarchical, and semantically explicit, AI systems extract it cleanly, interpret it correctly, and behave predictably.
...
### 12.2 The Market Math and the Moral Arithmetic
